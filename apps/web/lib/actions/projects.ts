...
export async function resetProjectAction(formData: FormData): Promise<void> {
  ...
  if (confirmation !== project.slug) {
    redirect(
      `/projects/${encodeURIComponent(project.slug)}/settings?error=${encodeURIComponent('Confirmation slug did not match. Type the project slug verbatim to confirm.')}`,
    );
  }
  ...
  try {
    ...
    redirect(
      `/projects/${encodeURIComponent(project.slug)}/settings?error=${encodeURIComponent((err as Error).message)}`,
    );
  }
  if (result === null) {
  ...
  redirect(`/projects/${encodeURIComponent(project.slug)}/settings?reset=1&summary=${encodeURIComponent(summary)}`);
}
...
export async function renameProjectAction(formData: FormData): Promise<void> {
  ...
  revalidatePath('/');
  redirect(`/projects/${encodeURIComponent(result.newSlug)}/settings?renamed=${encodeURIComponent(result.oldSlug)}`);
}
...
function settingsHref(identifier: string, errorCode: string, message: string): string {
  ...
  return `/projects/${encodeURIComponent(identifier)}/settings?${search.toString()}`;
}
