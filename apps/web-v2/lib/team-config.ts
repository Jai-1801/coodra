 ...
 * the local team config (`~/.contextos/config.json::team` block) so web
 * pages can branch on actual setup state instead of just env-var mode.
 *
 * The CLI package re-exports `readTeamConfig` via `@coodra/contextos-cli/lib/team-config`.
...
import { readTeamConfig as readTeamConfigCli, type TeamConfig } from '@coodra/contextos-cli/lib/team-config';

export type { TeamBlock, TeamConfig } from '@coodra/contextos-cli/lib/team-config';
 ...
 * resolver.
 *
 * Phase A intent (clarity-pass-plan, 2026-05-11): `~/.contextos/config.json::
 * mode` is the SOLE authority for machine mode. Env vars are derived from
 * config.json by `contextos team init` / `team leave`, never the other
 * way around.
 *
 * Pre-fix the function gated on `process.env.CONTEXTOS_MODE === 'team'`
 * AND a complete team block in config.json. That broke a real user flow:
 * after `team init`, the daemons spawn with `CONTEXTOS_MODE=team` (from
 * `~/.contextos/.env`), but the Next.js dev server inherits the operator's
 ...
 * team block in `~/.contextos/config.json` is the only signal that
 * matters. If a user wants to force-override (rare — e.g. testing),
 * they can either edit the file or set `CONTEXTOS_DEPLOYMENT=team-hosted`
