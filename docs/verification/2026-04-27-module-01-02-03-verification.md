...
| 6.2 + 6.3 cloud write + HNSW | PASS via narrow integration | `@coodra/contextos-db`'s `cloud-mode-write.test.ts` 2/2 PASS against the live Postgres. `pg_indexes`: `context_packs_embedding_hnsw_idx` exists, `USING hnsw (summary_embedding vector_cosine_ops) WITH (m='16', ef_construction='64')`. Synthetic 384-dim search ordering through `search_packs_nl` is impossible to test against Postgres because mcp-server is SQLite-only by design (F11). |
...
- Actual: `pnpm --filter @coodra/contextos-db run test:unit` → **42/42 passed**
...
Result: `pnpm --filter @coodra/contextos-db test:integration` reports `1 failed | 14 passed (15)` against a long-running compose Postgres. CI is green only because every CI job spins up a fresh service container. **The documented local-dev workflow (`pnpm -w docker:up && pnpm test:integration`) is broken.**
...
`runKeySegmentSchema` from `@coodra/contextos-shared/idempotency` is enforced at:
...
`apps/mcp-server/src/lib/db.ts` and `apps/hooks-bridge/src/lib/db.ts` always pass `{ kind: 'local' }` to `createDb`. M03 S4's commit message explicitly says "local services always write to local SQLite" and removed the prior `CONTEXTOS_DB_OVERRIDE_MODE` knob. The cloud-mode-write test directly constructs a `kind: 'cloud'` handle through `@coodra/contextos-db::createDb`, but the apps cannot.
