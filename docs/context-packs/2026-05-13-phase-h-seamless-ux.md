# Phase H — ContextOS seamless end-user UX
...
### H.2 — `contextos init` derives org_id from the verified Clerk JWT mirror
`packages/cli/src/commands/init.ts`:
- Pre-fix: `init` read `CONTEXTOS_TEAM_ORG_ID` from `~/.contextos/.env` and
  stamped projects with `__solo__` if the env var was unset, even in team
  mode.
- Post-fix: `init` calls `readVerifiedToken({ homeOverride: contextosHome })`
...
- New `upsertEnvKey(envPath, key, value)` — idempotent atomic-rename merge of
  arbitrary key=value pairs into `~/.contextos/.env`. Used to land
  `CONTEXTOS_INVITE_HMAC_SECRET`, `CLERK_SECRET_KEY`, `CLERK_PUBLISHABLE_KEY`
  alongside the four primary keys.
- `readExistingExtras(envPath)` — preserves an existing
  `CONTEXTOS_INVITE_HMAC_SECRET` on wizard re-runs so previously-minted invite
...
- `apps/web-v2/next.config.ts` (unchanged from Phase F.6+) already layers
  `~/.contextos/.env` into `process.env` with the override semantics that make
...
- After the org is picked, the wizard calls `ensureContextosCliJwtTemplate`
...
### H.5 — top-level `contextos invite <email>` command
- New `packages/cli/src/commands/invite.ts` + `packages/cli/src/lib/invite-mint.ts`.
- Reads `CONTEXTOS_INVITE_HMAC_SECRET` from `~/.contextos/.env`, signs the
...
- `apps/web-v2/lib/actions/invite.ts::mintInviteAction` no longer calls
  ...
  The admin's `/settings/team` flow now mints purely a ContextOS HMAC token
...
- `apps/web-v2/app/install/[token]/cli.sh/route.ts`: the one-line curl-piped
  installer now invokes `contextos team join <invite-url>` (Phase G
  browser-handoff path) instead of `contextos team install --bootstrap-url`
  ...
  contextos feature add my-first-thing"`.
...
- Default `waitTimeoutMs` raised from 10000 to 30000. Phase G live test
  ...
  (CONTEXTOS_HOME resolution + SQLite init + tool-registry load); 10s was a
  false-negative.
- launchd label hashing (gap 17) deferred — documented as a non-goal for
  Phase H. Users running multiple `CONTEXTOS_HOME` in parallel should
  `contextos stop` before swapping homes.
...
### H.11 — `contextos sync retry` (deferred)
- Not on the 8-acceptance-test critical path. The `pending_jobs` retry
  surface is observable today via `contextos doctor --full`'s outbox depth
  checks. A dedicated retry command lands in Phase H+1.

### H.12 — team-init auto-creates `contextos_cli` Clerk JWT template
`packages/cli/src/lib/team-init/clerk-jwt-template.ts` (new):
- `ensureContextosCliJwtTemplate({ secretKey })` — idempotent. GETs
  `https://api.clerk.com/v1/jwt_templates`, checks for an existing
  `contextos_cli`, POSTs to create only when absent.
- Claims template:
  ```
  org_id   = {{org.id}}
  org_role = {{org.role}}
...
- `pnpm --filter @coodra/contextos-cli build` clean (CLI dist + bundled
  runtime for mcp-server / hooks-bridge / sync-daemon).
- **Test 1 smoke** verified twice from clean state (idempotency):
  1. `npm i -g @coodra/contextos-cli` → simulated via local dist.
  2. `contextos init` → exits 0, registers project with `org_id='__solo__'`,
     seeds 25 policy rules, writes baseline `.contextos.json` + `.mcp.json`
     + `.env`.
  3. `contextos feature add greet --description "Say hi"` → exits 0,
     `docs/features/greet/feature.md` exists, `~/.contextos/data.db`
...
is **idempotent** when repeated from clean state (`contextos logout && rm -rf
~/.contextos/clerk-token.json` between runs; delete cloud rows for
...
`contextos_cli` JWT template → generates LOCAL_HOOK_SECRET +
CONTEXTOS_INVITE_HMAC_SECRET → writes `config.json` + `.env` → opens browser
...
✓ Team setup complete — machine flipped to team mode.
  ...
  …
Next steps:
  1. `contextos start` — daemons pick up team-mode env; sync-daemon spawns now.
  2. `contextos invite <email>` — share invite links with teammates.
  3. Open http://localhost:3001/ once daemons are up — admin dashboard.
```

### Test 3 — admin invites teammate
```bash
contextos invite jane@example.com
```
Expected output:
```
...
```
✓ Welcome Jane! Try: contextos feature add my-first-thing
```
...
Admin and Jane each run `contextos feature add <slug>` from their respective
...
Admin edits `~/.contextos/config.json::team.clerkUserId` to `"user_FAKE"`,
then runs `contextos feature add tamper-test`. Web `/features` shows the row
...
(claim-cache TTL), Jane runs `contextos feature add viewer-attempt`. CLI
exits non-zero with:
```
contextos feature: this row cannot be authored — your role 'viewer' lacks write privilege.
```
...
contextos feature add solo-only
...
contextos feature add team-again
