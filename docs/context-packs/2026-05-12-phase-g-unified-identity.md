...
> `solo | team`; the laptop-vs-server distinction (CONTEXTOS_DEPLOYMENT
...
1. CLI writes claimed identity from `config.json::team.clerkUserId`
   without verification. Anyone with write access to `~/.contextos/`
   could forge any user's attribution.
2. `contextos team install` didn't actually authenticate — the redeem
...
- `packages/shared/src/auth/clerk-token-store.ts` — NEW. Disk I/O for
  `~/.contextos/clerk-token.json`. `writeToken / readVerifiedToken /
...
- `apps/web-v2/app/auth/cli-login/page.tsx` — NEW. Browser-handoff
  ...
  'contextos_cli' })`, and redirects to `http://127.0.0.1:<port>/?token=<jwt>&state=<state>`.
...
### G.3 — `contextos login` CLI command
...
- Wired in `program.ts` as both `contextos login` (top-level) and
  `contextos team login` (backward-compat alias).
...
### G.4 — `contextos logout` CLI command
...
### G.8 — Drop CONTEXTOS_DEPLOYMENT, unify web modes
...
- `apps/web-v2/middleware.ts` — SIMPLIFIED. Mode resolves from
  `CONTEXTOS_MODE` env var. No more `CONTEXTOS_DEPLOYMENT` for
  identity decisions.
...
### G.10 — `contextos org` (status + switch)
...
- Wired in `program.ts` as `contextos org status` + `contextos org switch <slug>`.
...
- `@coodra/contextos-shared`: 249 (+**41**)
- `@coodra/contextos-mcp-server`: 266 (+**9**)
- `@coodra/contextos-hooks-bridge`: 75 (+**8**)
- `@coodra/contextos-web-v2`: 32 (+**13**)
- `@coodra/contextos-cli`: 269 (+**21**)
...
- Recovery: `contextos login` — browser handoff, Clerk sign-in, token
  written, done
...
1. Apply migrations 0018 (postgres) + 0016 (sqlite) to their dev DB:
   ...
   ```
2. Configure the `contextos_cli` JWT template in their Clerk dashboard
   (name + 24h TTL + include org_id/org_role/email claims).
3. Run `contextos team init` from a fresh `~/.contextos`-like state,
...
7. ❌ Multi-org user trapped in one org — `contextos org switch`.
