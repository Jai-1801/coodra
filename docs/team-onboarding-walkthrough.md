...
This document walks you through onboarding ContextOS team mode end-to-end as a real user would, from scratch. It's the verified sequence of commands the bug-hunt + smoke-test pass produced — every step has been executed against a real Supabase project.

If something goes wrong, every command surfaces a clear error with a `hint:` line. Don't stress. Worst case you `contextos team leave --yes` and start over.
...
- The current ContextOS CLI built locally OR `npm i -g @coodra/contextos-cli`.
...
alias contextos="node $PWD/packages/cli/dist/index.js"   # for this shell only
contextos --version   # should print 0.1.0
```

(Once you ship via npm, replace this with `npm i -g @coodra/contextos-cli`.)
...
## Step 3 — Run `contextos team setup` (admin, one-time)
...
```
contextos team setup — bootstrapping team Postgres at postgresql://postgres:***@db.abcdefgh.supabase.co:5432/postgres
  ...
  ✓ 14 tables present
  ✓ local config promoted to team mode (~/.contextos/config.json)
  ✓ ~/.contextos/.env updated (CONTEXTOS_MODE=team, DATABASE_URL, LOCAL_HOOK_SECRET)
...
- Wrote `~/.contextos/config.json::team` (CLI's source of truth).
- Wrote `~/.contextos/.env` with `CONTEXTOS_MODE=team`, `DATABASE_URL`, `LOCAL_HOOK_SECRET`, `CONTEXTOS_TEAM_ORG_ID` so `contextos start` launches in team mode.

### If it fails

- **"missing user id / org id / database url"** → set them via `--flag` or via env vars `CONTEXTOS_TEAM_USER_ID`, `CONTEXTOS_TEAM_ORG_ID`, `DATABASE_URL`.
...
```bash
mkdir ~/contextos-demo
cd ~/contextos-demo
git init
echo '# demo' > README.md
git add . && git commit -m 'init'

contextos init --project-slug demo-app
```

Expected output:

```
✓ Detected project root: /Users/<you>/contextos-demo
✓ Detected IDEs: claude, cursor, windsurf
✓ Resolved ContextOS home: /Users/<you>/.contextos
...
✓ Resolved mcp-server runtime: bundled
  ...
  + .contextos.json (wrote projectSlug='demo-app')
  + .mcp.json (created baseline .mcp.json with contextos entry)
  + .env (created baseline .env)
  ! ~/.claude/settings.json (overwrote ContextOS hook entries)
  + docs/feature-packs/demo-app/{meta.json,spec.md,implementation.md,techstack.md}

ContextOS is ready (project 'demo-app').
  → Restart your IDE so it picks up .mcp.json.
  → Run `contextos doctor` to verify the install.
  → Run `contextos start` to launch the MCP server + Hooks Bridge daemons.
```

### What `init` wrote

- `.contextos.json` — pins the project slug.
- `.mcp.json` — Claude Code / Cursor / Windsurf use this to spawn the ContextOS MCP server.
- `.env` — Clerk sentinels + `LOCAL_HOOK_SECRET` + ports. **Note**: `CONTEXTOS_MODE` is intentionally NOT in this file (per Phase 4 H5) — your `~/.contextos/.env::CONTEXTOS_MODE=team` from `team setup` governs.
- `~/.claude/settings.json` — wires ContextOS hooks into Claude Code's hook events.
- `docs/feature-packs/demo-app/` — seed feature pack the agent reads at session start.

---

## Step 5 — Verify the install with `contextos doctor`

```bash
contextos doctor --full
```
...
**Check 26 yellow is expected** for a fresh team setup — cloud has no runs yet because you haven't started a session. The remediation tells you to run `contextos start` (which the next step does).
...
- **MCP Server** at `127.0.0.1:3100` — exposes the `contextos__*` tools to Claude Code.
...
You should see ~3 lines confirming each daemon started. Daemon logs go to `~/.contextos/logs/`.
...
2. Open `~/contextos-demo` in Claude Code.
3. Start a session. Try a basic prompt: *"Edit README.md to add a project description."*
4. Claude Code's hooks fire on `SessionStart`, `PreToolUse`, `PostToolUse`, `SessionEnd`. The bridge audits each event into local SQLite. The agent uses the `contextos__*` MCP tools to record decisions and save context packs.

Watch the logs:

```bash
tail -f ~/.contextos/logs/hooks-bridge.log
tail -f ~/.contextos/logs/mcp-server.log
tail -f ~/.contextos/logs/sync-daemon.log
```
...
cd ~/their-project
contextos init --project-slug their-project
contextos start
```
...
2. On their machine, wait ~10s, then run:
   ```bash
   sqlite3 ~/.contextos/data.db "SELECT description, created_by_user_id FROM decisions ORDER BY created_at DESC LIMIT 1"
   ```
...
|---|---|---|
| `contextos start` fails: "DATABASE_URL is required" | `team setup` wasn't run, or `~/.contextos/.env` was deleted | Re-run `contextos team setup ...` |
...
| Check 26 stays yellow forever | Sync daemon not running or wedged | `contextos status` (is sync-daemon listed?), check `~/.contextos/logs/sync-daemon.log` |
| Decisions don't show up on teammate's machine | Pull-tick hasn't fired yet (10s window) OR sync-daemon down on writer | Wait 10s; check `contextos doctor --full`'s sync checks (24-27) |
| `contextos doctor` says CONTEXTOS_MODE != team | Project `.env` overrode home `.env` | Check your project `.env` doesn't have `CONTEXTOS_MODE=solo` (post-Phase-4 init no longer writes this; if you have a legacy `.env`, delete the line) |
| Need to undo a migration | `contextos team migrate --rollback` | Restores local snapshot + deletes cloud rows from this attempt |
| Need to leave team mode | `contextos team leave --yes` | Demotes config back to solo. Cloud data untouched (other team members still see it). |
...
- `/onboarding/connect` — auto-detects whether the user has run `contextos team join` and shows a green check.
