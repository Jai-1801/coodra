...
| Worker substrate | `@contextos/cli/lib/outbox` — landed in M03.1 | OutboxWorker class + lease/retry/dead-letter; reused with `queueKind: 'sync_to_cloud'` |
| Logger | `pino` (via `@contextos/shared`) — pinned by M01 | Same structured-log shape across the fleet |
...
| `turbo build --filter=@contextos/<service>` | Existing build pipeline; no new tool |
