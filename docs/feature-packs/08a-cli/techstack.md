...
| `zod` | `^4.3.6` (workspace pin) | Validating `.mcp.json`, `meta.json`, the `team join` token shape, env vars. Same pin as `@coodra/contextos-shared`. |
...
| `@coodra/contextos-shared` | workspace | Logger, errors, env validation. The CLI MUST use the same pino logger and the same error hierarchy as every other package. |
| `@coodra/contextos-db` | workspace | The `doctor` check that probes `~/.contextos/data.db` reuses the existing SQLite open path so behavior matches what the MCP server expects. |
...
- **`npx @coodra/contextos-cli init` resolves the bin via `process.execPath` for the `.mcp.json` `command` field.** This means the `.mcp.json` written by an `npx` invocation embeds a path under the npx cache, which can be garbage-collected by npm. The `init` command MUST detect this case and warn YELLOW in `doctor`'s "MCP config validity" check, recommending a global install (`npm i -g @coodra/contextos-cli`) for stability. (Tracked as a follow-up: rewrite the `.mcp.json` `command` to `npx @coodra/contextos-cli mcp-stdio` so the path is resolved at IDE-startup time, not at `init` time. Out of 08a scope.)
