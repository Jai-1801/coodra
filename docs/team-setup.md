...
ContextOS team mode is **bring-your-own-database**. Each team owns their data: their own Supabase project (or any Postgres ≥ 16 with pgvector available), their own deploy of the web app, their own credentials. ContextOS does not host or proxy any cloud database on the team's behalf.
...
3. The `contextos` CLI installed: `npm install -g @coodra/contextos-cli`.
4. ~5 minutes.
...
## Step 3 — Run `contextos team setup`

This command:
1. Verifies the Postgres connection works.
2. Installs `pgvector` (`CREATE EXTENSION IF NOT EXISTS vector`).
3. Applies the ContextOS schema (Drizzle migrations, idempotent).
4. Generates a 32-byte hex local hook secret.
5. Writes your local `~/.contextos/config.json` for team mode.
...
- **Run the extension install manually** as a superuser, then retry with `--skip-pgvector`:
  ...
  Then:
  ```bash
  contextos team setup --skip-pgvector --user-id ... --org-id ... --database-url ...
  ```
...
If you've been using ContextOS solo mode and want to move that history to the team:

```bash
contextos team migrate --yes \
  --user-id 'user_2abcXYZ...' \
  --org-id 'org_2abcXYZ...' \
...
The migrate command:
- Snapshots your local `~/.contextos/data.db` to `~/.contextos/data.db.pre-migrate-{timestamp}` before any destructive work.
...
- `contextos team migrate --rollback` deletes the partially-written cloud rows and restores your local snapshot.
- `contextos team migrate --resume` continues from the last successfully-completed phase.
...
Each member of the team, on each machine they want to use ContextOS from:

```bash
# 1. Install the CLI
npm install -g @coodra/contextos-cli
...
This writes their `~/.contextos/config.json` to team mode. The next time they:
...
`CONTEXTOS_MODE` controls the mode env-side. Set it in `~/.contextos/.env` or your shell:
```
CONTEXTOS_MODE=team
DATABASE_URL=<your url>
```
Restart `contextos start` so the daemons pick up the change.

### "My teammate joined but I don't see their decisions in my Claude Code session"

The sync daemon's pull-tick runs every 10s. Decisions land in cloud immediately on the writer's side (via the audit outbox), but a 0–10s window exists where the reader's local SQLite hasn't pulled yet. Run `contextos doctor` and look at check 24 / 26 / 27 to verify sync is healthy.

If that doesn't help, manually trigger a pull from your machine:
```bash
contextos status  # will show last sync tick
```

### "I want to leave the team and go back to solo"

```bash
contextos team leave --yes
```

This:
- Demotes your local `~/.contextos/config.json` back to solo mode.
- Does NOT delete team-tagged rows from your local SQLite (they remain as historical state — a future `contextos clean-team-data` will offer scrubbing).
- Does NOT touch cloud data (other team members still see everything).

### "I want to rotate my team's local hook secret"

Re-run `contextos team setup` with `--secret '<new-hex-string>'`. This overwrites the local config block with the new secret. Distribute the new secret to teammates and have them re-join with `contextos team join --secret '<new>'`. Old secret continues to work until everyone has cycled (the secret is a per-machine bearer token, not enforced cluster-side yet).

---

## Cost / data ownership

- **Where your data lives**: in your Supabase project. ContextOS does not store team data on any infrastructure we operate.
