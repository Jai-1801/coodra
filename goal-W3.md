...
(3) `pnpm pack` produces `coodra-contextos-cli-0.1.0-beta.2.tgz`. Print its byte size + file count + the `npm pack --dry-run` line that lists the file count.
...
(5) Print the EXACT publish command the user runs:
       `cd /Users/abishaikc/Coodra/packages/cli && pnpm publish coodra-contextos-cli-0.1.0-beta.2.tgz --tag beta --no-git-checks --otp <6-digit-OTP-from-authenticator>`
...
(6) After the user pastes a publish success line containing `+ @coodra/contextos-cli@0.1.0-beta.2`, continue:

Agent post-publish verification:
(7) `npm view @coodra/contextos-cli@beta version` prints `0.1.0-beta.2`.
(8) Public-registry install on this machine (simulates a fresh user):
       `npm uninstall -g @coodra/contextos-cli 2>/dev/null || true`
       `npm install -g @coodra/contextos-cli@beta`
       `contextos --version` prints `0.1.0-beta.2`.
(9) In a fresh /tmp project: `contextos init && contextos start --no-open`. Wait for daemons. `curl -sSf http://127.0.0.1:3001/api/healthz` returns ok.
(10) Steps 8+9 twice from clean state (`npm uninstall -g`, `rm -rf /tmp/h-w3-*` between runs). Paste both runs.

Acceptance: public-registry install of 0.1.0-beta.2 produces a working `contextos start` with web reachable on :3001 — proved by the agent's curl output. Twice clean.
