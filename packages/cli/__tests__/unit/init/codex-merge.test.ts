 ...
 *   6. removeCodexConfig strips only the contextos entry.
 */

const ENTRY: ContextosMcpEntry = {
  command: 'node',
  args: ['/abs/path/runtime/mcp-server/index.js', '--transport', 'stdio'],
  env: { CONTEXTOS_LOG_DESTINATION: 'stderr', CLERK_SECRET_KEY: 'sk_test_x' },
};
...
describe('mergeCodexConfig — Codex .codex/config.toml writer', () => {
  ...
  beforeEach(async () => {
    cwd = await mkdtemp(join(tmpdir(), 'contextos-codex-merge-'));
  });
  afterEach(() => {
    /* tmp cleaned by OS */
  });

  it('greenfield: creates .codex/config.toml with the contextos MCP entry', async () => {
    ...
    expect(isCodexEntryEqual(ENTRY, servers.contextos)).toBe(true);
  });
  ...
  it('preserves a drifted contextos entry without --force', async () => {
    ...
    const parsed = parseToml(await readFile(join(cwd, '.codex', 'config.toml'), 'utf8')) as Record<string, unknown>;
    expect((parsed.mcp_servers as Record<string, Record<string, unknown>>).contextos.command).toBe('custom-node');
  });

  it('--force overwrites a drifted contextos entry with the baseline', async () => {
    await mkdir(join(cwd, '.codex'), { recursive: true });
    await writeFile(join(cwd, '.codex', 'config.toml'), '[mcp_servers.contextos]\ncommand = "custom"\n', 'utf8');
    const result = await mergeCodexConfig({ cwd, entry: ENTRY, force: true, dryRun: false });
    expect(result.action).toBe('forced');
    const parsed = parseToml(await readFile(join(cwd, '.codex', 'config.toml'), 'utf8')) as Record<string, unknown>;
    expect(isCodexEntryEqual(ENTRY, (parsed.mcp_servers as Record<string, unknown>).contextos)).toBe(true);
  });
  ...
  it('removeCodexConfig strips only the contextos entry', async () => {
    await mkdir(join(cwd, '.codex'), { recursive: true });
    await writeFile(
      join(cwd, '.codex', 'config.toml'),
      'model = "gpt-5"\n\n[mcp_servers.othersrv]\ncommand = "other"\n',
    ...
    expect(parsed.model).toBe('gpt-5');
    const servers = parsed.mcp_servers as Record<string, unknown>;
    expect(servers.contextos).toBeUndefined();
