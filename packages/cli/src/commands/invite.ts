 ...
 *
 * Top-level `contextos invite <email>` command. The admin's one-command
 ...
 *   3. Read DATABASE_URL + CONTEXTOS_INVITE_HMAC_SECRET from ~/.contextos/.env.
 ...
 *   "contextos invite jane@example.com" must mint a single shareable
...
export interface InviteOptions {
  ...
  /** Override `CONTEXTOS_PUBLIC_URL` env resolution. */
...
export async function runInviteCommand(
  ...
  if (email.length === 0 || !isValidEmail(email)) {
    io.writeStderr(
      `${pc.red('contextos invite')}: invalid email "${rawEmail}". Expected a shape like name@domain.tld.\n`,
    );
  ...
  if (!VALID_ROLES.has(role)) {
    io.writeStderr(
      `${pc.red('contextos invite')}: invalid role "${options.role}". Expected one of: admin, member, viewer.\n`,
    );
    return io.exit(EXIT_USER_ACTION_REQUIRED);
  }
  ...
  if (teamCfg.mode !== 'team') {
    io.writeStderr(
      `${pc.red('contextos invite')}: this machine is in solo mode. Run \`contextos team init\` first to set up a team.\n`,
    );
    return io.exit(EXIT_USER_ACTION_REQUIRED);
  }

  // Layer ~/.contextos/.env into process.env so DATABASE_URL is visible
  ...
  if (verified === null) {
    io.writeStderr(
      `${pc.red('contextos invite')}: no verified Clerk session. Run \`contextos login\` first (or your existing token may have expired).\n`,
    );
    return io.exit(EXIT_USER_ACTION_REQUIRED);
  }
  ...
  if (verified.role !== 'admin') {
    io.writeStderr(
      `${pc.red('contextos invite')}: only org admins can mint invites. Your verified role is "${verified.role}".\n`,
    );
    return io.exit(EXIT_USER_ACTION_REQUIRED);
  }
  ...
  if (envBlock === null) {
    io.writeStderr(
      `${pc.red('contextos invite')}: ~/.contextos/.env is missing or incomplete. Re-run \`contextos team init\` to repair it.\n`,
    );
    return io.exit(EXIT_USER_RECOVERABLE);
  }

  // Resolve baseUrl. Precedence: --web-url > CONTEXTOS_PUBLIC_URL env >
  // CONTEXTOS_WEB_URL env > http://localhost:3001 default.
  const procEnv = options.env ?? process.env;
  const baseUrl = (
    options.webUrl ??
    procEnv.CONTEXTOS_PUBLIC_URL ??
    procEnv.CONTEXTOS_WEB_URL ??
    'http://localhost:3001'
  ).replace(/\/$/, '');
  ...
  try {
    ...
    if (err instanceof InviteSecretMissingError) {
      io.writeStderr(`${pc.red('contextos invite')}: ${err.message}\n`);
      return io.exit(EXIT_USER_ACTION_REQUIRED);
    }
    io.writeStderr(`${pc.red('contextos invite')}: mint failed — ${(err as Error).message}\n`);
    return io.exit(EXIT_USER_RECOVERABLE);
  }
