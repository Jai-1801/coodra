...
> This is the final piece of ContextOS shipping. Phase H closed the UX gaps
> at the CLI surface; this initiative closes the gap at the WEB surface so
> a single `npm i -g @coodra/contextos-cli@latest` gives a fully working
> ContextOS in BOTH solo and team mode on ANY machine with Node ≥ 22.
>
...
### Solo user
```bash
npm i -g @coodra/contextos-cli@latest
mkdir myproject && cd myproject && git init
contextos init
contextos start
# → daemons up, browser opens to http://localhost:3001/
# → user sees their solo dashboard with /features, /decisions, /packs
# → all data lives in ~/.contextos/data.db (local-first)
```

### Admin spinning up a team
```bash
contextos team init       # wizard handles Postgres, Clerk, browser sign-in
contextos start --tunnel  # web + Cloudflare quick-tunnel public URL
contextos invite jane@acme.com
...
```
@coodra/contextos-cli (npm tarball, ~30-50 MB)
...
`contextos start` already iterates over service descriptors and spawns each
...
- `contextos start` spawns it like the other daemons.
- Native bindings (`better-sqlite3`, `sqlite-vec`) work in standalone (the largest risk to manage).
- Acceptance: in isolated /tmp install, `contextos init && contextos start` → `curl localhost:3001/` returns 200. Twice clean.
...
- No 500s. No "TypeError can't read ~/.contextos/data.db". No native-binding crashes.
...
- User publishes; pastes the npm "+ @coodra/contextos-cli@0.1.0-beta.2" output to confirm.
- Agent then `npm i -g @coodra/contextos-cli@beta` from public registry on this machine; runs Test 1 + boots web. Twice clean.
- Acceptance: web boots from public-registry install just like the local-tarball install did.

### W4 — Cross-machine via Cloudflare Tunnel (`contextos start --tunnel`)
- Add `--tunnel` flag to `contextos start`.
- If `cloudflared` is on PATH, spawn quick-tunnel; parse `*.trycloudflare.com` URL.
- Atomically write `CONTEXTOS_PUBLIC_URL=<tunnel-url>` to `~/.contextos/.env`.
- Print install instructions if `cloudflared` missing; non-fatal (local web still up).
- `contextos invite` URLs use the tunnel host.
- `contextos stop` cleans up the tunnel + reverts CONTEXTOS_PUBLIC_URL.
...
- The npm package name (`@coodra/contextos-cli`) — locked.
...
- For W3 specifically: `npm uninstall -g @coodra/contextos-cli` between public-registry runs.
...
3. **After all 4 goals green**: promote `beta.2` → `latest` if satisfied:
   ```bash
   npm dist-tag add @coodra/contextos-cli@0.1.0-beta.2 latest
   ```
...
- **`CONTEXTOS_PUBLIC_URL=http://localhost:3001`** baked into invites: replaced with the tunnel URL when `--tunnel` is set.
...
After W1-W4, the CLI is a true single-binary install of the entire ContextOS
