...
| MCP client can't connect | Check `MCP_SERVER_PORT` in `.env`. Ensure the server is running (`pnpm --filter @coodra/contextos-mcp-server dev`). |
| Hooks bridge returns 500 | Check logs with `pino-pretty`: `pnpm --filter @coodra/contextos-hooks-bridge dev \| npx pino-pretty`. |
